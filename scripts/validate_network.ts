 ...
 */
function generateSyntheticData(numSamples: number, numClasses: number, inputSize: number): {
  inputs: number[][][],
  targets: number[][][]
} {
  const inputs: number[][][] = [];
 ...
 */
function createClassificationModel(inputSize: number, hiddenSize: number, numClasses: number): SequentialModel {
  const model = new SequentialModel('test_model', 'Modelo de Prueba para Clasificación');
 ...
 */
function evaluateModel(model: SequentialModel, testInputs: number[][][], testTargets: number[][][]): void {
  ...
  console.log(`Predicciones correctas: ${correctCount} de ${testInputs.length}`);
  
  // Mostrar matriz de confusión
  console.log('\nMatriz de confusión:');
  console.log('Filas: clase real, Columnas: clase predicha');
  console.log('     ' + CLASSES.map(c => c.substring(0, 5).padEnd(7)).join(''));
  ...
  for (let i = 0; i < numClasses; i++) {
    ...
    const totalPredicted = confusionMatrix.reduce((sum, row) => sum + row[i], 0);
    ...
    console.log(`${CLASSES[i].padEnd(10)}: Precisión=${(precision * 100).toFixed(2)}%, Recall=${(recall * 100).toFixed(2)}%, F1=${(f1 * 100).toFixed(2)}%`);
  }
}
...
async function validateNeuralNetwork(): Promise<void> {
  ...
  const { inputs: trainInputs, targets: trainTargets } = generateSyntheticData(numTrainSamples, numClasses, inputSize);
  const { inputs: testInputs, targets: testTargets } = generateSyntheticData(numTestSamples, numClasses, inputSize);
  ...
  const trainingResults = await model.train(trainInputs, trainTargets, epochs, batchSize);
  const trainingTime = (Date.now() - startTime) / 1000; // en segundos
  
  console.log(`\nEntrenamiento completado en ${trainingTime.toFixed(2)} segundos`);
  console.log(`Pérdida final: ${trainingResults.loss[trainingResults.loss.length - 1].toFixed(4)}`);
  console.log(`Precisión final: ${(trainingResults.accuracy[trainingResults.accuracy.length - 1] * 100).toFixed(2)}%`);
  ...
  fs.writeFileSync(resultsPath, JSON.stringify({
    ...
    finalAccuracy: trainingResults.accuracy[trainingResults.accuracy.length - 1],
