...
export class TrainingController {
    ...
    try {
      ...
      if (!labels) {
        res.status(400).json({
          success: false,
          error: {
            message: 'Se requieren etiquetas para las imágenes de entrenamiento',
            code: 400
          }
        });
        return;
      ...
      res.status(500).json({
        success: false,
        error: {
          message: error instanceof Error ? error.message : 'Error al iniciar el entrenamiento',
          code: 500
        }
      });
    }
   ...
   * @param res Respuesta HTTP
   */
  public getTrainingStatus = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const status = await this.trainingService.getTrainingStatus(trainingId as string);
      ...
      res.status(500).json({
        success: false,
        error: {
          message: error instanceof Error ? error.message : 'Error al obtener estado del entrenamiento',
          code: 500
        }
      });
    }
  ...
  public saveModel = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await this.trainingService.saveModel(modelName, description);
      ...
      res.status(500).json({
        success: false,
        error: {
          message: error instanceof Error ? error.message : 'Error al guardar el modelo',
          code: 500
        }
      });
    }
  ...
  public loadModel = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(500).json({
        success: false,
        error: {
          message: error instanceof Error ? error.message : 'Error al cargar el modelo',
          code: 500
        }
      });
    }
